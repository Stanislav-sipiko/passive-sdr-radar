...
### Термин «подключение через фазированный делитель» (или phased power divider / power splitter)
означает способ распределения сигнала от одного приёмопередатчика (или приёмника) на несколько антенн, при этом:

каждая антенна получает одинаковую амплитуду сигнала,

фазы всех сигналов согласованы (in-phase),

и, как следствие, все антенны работают как единый фазированный массив, усиливая сигнал в заданном направлении.

## 📡 Что делает фазированный делитель

Это устройство делит входной сигнал (например, от приёмного модуля) на N выходов, сохраняя одинаковую фазу и амплитуду.
Примеры:

1→2 делитель — два выхода, 3 dB потерь;

1→4 делитель — четыре выхода, 6 dB потерь;

1→8 и т.д.

Если длины кабелей от делителя до антенн равны, все сигналы приходят к антеннам в фазе, и диаграмма направленности усиливается в их общем направлении.

## ⚙️ Типы делителей
Тип	Описание	Применение
Wilkinson power divider	Согласованный делитель с изоляцией между портами, используется в ВЧ схемах.	Лабораторный / внутри корпуса
Hybrid (180°/90° hybrid)	Позволяет создавать фазовые сдвиги между выходами.	Фазированные решётки
Коаксиальный T-сплиттер	Простое ответвление коаксиала, при одинаковых длинах — даёт in-phase выходы.	Практический вариант для 2–4 антенн
Corporate feed network	Сеть делителей (1→2→4→8) с равными длинами ветвей.	Классическая схема LPDA/радар массивов
🔧 Пример: подключение 4 антенн LPDA
                ┌──► ANT1
Input ──┬───────┤
         │       └──► ANT2
         │
...
Все кабели от делителя до антенн одинаковой длины (±1 см).

Центральный узел (делитель) — это 1→4 Wilkinson или «звезда» на 4 ответвления.

Если длины равны, все сигналы приходят синфазно → суммарный усиленный луч (Gain +6 dB при 4 антеннах).

## 🧠 Пример из практики

Для пассивного радара — 4 приёмные LPDA направлены в одну сторону.

Делитель принимает сигнал с антенн или распределяет опорный сигнал (в зависимости от архитектуры).

Используются коаксиалы RG-400/LMR-240 одинаковой длины.

Согласование 50 Ω на всех портах.

Фазовые ошибки >5° уже портят суммирование, поэтому физическая симметрия критична.

## ⚡ Резюме

Фазированный делитель нужен, чтобы все антенны:

питались одинаково по амплитуде,

работали синфазно,

создавали усиленное направленное излучение или приём.

Это ключевой элемент при построении массивов из нескольких LPDA или Yagi, чтобы они работали как единая антенна с высоким усилением.
